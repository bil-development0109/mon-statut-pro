...
var Store = class _Store {
  ...
  async getMetadata(key) {
    const res = await this.client.makeRequest({ key, method: "head" /* HEAD */, storeName: this.name });
    if (res.status === 404) {
      return null;
    }
...
var BlobsServer = class _BlobsServer {
  ...
  async delete(req, res) {
    ...
    try {
      await import_node_fs.promises.rm(dataPath, { recursive: true });
    } catch (error) {
  ...
  async get(req, res) {
    ...
    const stream = (0, import_node_fs.createReadStream)(dataPath);
  ...
  async put(req, res) {
    ...
    try {
      ...
      await new Promise((resolve2, reject) => {
        req.pipe((0, import_node_fs.createWriteStream)(tempDataPath));
        req.on("end", resolve2);
        req.on("error", reject);
      });
      await import_node_fs.promises.mkdir((0, import_node_path.dirname)(dataPath), { recursive: true });
      await import_node_fs.promises.rename(tempDataPath, dataPath);
